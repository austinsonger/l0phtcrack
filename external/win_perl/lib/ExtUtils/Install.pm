...
sub _unlink_or_rename { #XXX OS-SPECIFIC
    ...
    # https://bugs.activestate.com/show_bug.cgi?id=85799
    # On HP-UX, you can't unlink in-use object files, similar to Windows DLLs.
    # However, renaming them appears to be the correct solution to the problem,
    # and we will just need to remove the .AAA copy later on.
    if ( $^O eq 'hpux' && $tryhard ) {
        warn "WARNING: Unable to unlink '$file': $error\n",
             "Going to try to rename it to '$tmp'.\n";

        if ( rename $file, $tmp ) {
            warn "Rename successful. NOTE: You will need to manually remove '$tmp'.\n";
            return $file;
        }
        else {
