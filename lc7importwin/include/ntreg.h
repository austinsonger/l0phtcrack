#ifndef __INC_NTREG_H
#define __INC_NTREG_H
...
struct sk_key {
	...
	int16_t id;          /* 0x0000	Word	ID: ASCII-"sk" = 0x6B73        */
	int16_t dummy1;      /* 0x0002	Word	Unused                         */
...
struct lf_key {
	...
	int16_t id;         /* 0x0000	Word	ID: ASCII-"lf" = 0x666C or "lh" = 0x686c */
	int16_t no_keys;    /* 0x0002	Word	number of keys          */
	/* 0x0004	????	Hash-Records            */
...
struct li_key {
	...
	int16_t id;		/* 0x0000	Word	ID: ASCII-"li" = 0x696C	*/
	int16_t no_keys;	/* 0x0002	Word	number of keys		*/
	/* 0x0004	????	Hash-Records		*/
	struct li_hash {
		int32_t ofs_nk;	/* 0x0000	D-Word	Offset of corresponding "nk"-Record  */
...
struct db_key {
	...
	int16_t id;		/* 0x0000	Word	ID: ASCII-"li" = 0x6462		*/
	int16_t no_part;	/* 0x0002	Word	number of data parts		*/
...
struct ri_key {
	...
	int16_t id;		/* 0x0000	Word	ID: ASCII-"ri" = 0x6972	*/
	int16_t no_lis;		/* 0x0002	Word	number of pointers to li */
	/* 0x0004	????	Hash-Records		*/
	struct ri_hash {
...
struct vk_key {
	...
	int16_t flag;		/* 0x0010	Word	Flag
						0x1 ANSI encoding		*/
	int16_t dummy1;		/* 0x0012	Word	Unused (data-trash)	*/
...
struct nk_key {
	...
	/* Offset	Size	Contents */
	int16_t id;		 /*  0x0000	Word	ID: ASCII-"nk" = 0x6B6E			*/
	int16_t type;		 /*  0x0002	Word	for the root-key: 0x2C, otherwise 0x20	*/
	...
	int16_t len_name;	 /*  0x0048	Word	name-length				*/
	int16_t len_classnam;	 /*  0x004A	Word	class-name length			*/
...
#define ALLOC(result, size, number)\
    { \
        if (!((*(void **)(&result)) = (void *) calloc ((number), (size)))) { \
            abort(); \
		       } \
    }
...
int ex_next_n(const struct hive * const hdesc, int nkofs, int * const  count, int * const  countri, struct ex_data * const  sptr);
...
struct keyval *get_val2buf(struct hive *hdesc, struct keyval *kv, int vofs, const char *path, int type, int exact);
int get_dword(struct hive *hdesc, int vofs, const char *path, int exact);
int put_buf2val(struct hive *hdesc, struct keyval *kv, int vofs, const char *path, int type, int exact);
int put_dword(struct hive *hdesc, int vofs, const char *path, int exact, int dword);
void export_key(struct hive *hdesc, int nkofs, const char *name, const char *filename, const char *prefix);
