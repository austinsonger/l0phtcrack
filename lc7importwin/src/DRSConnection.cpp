...
DrsConnection::DrsConnection(RPC_BINDING_HANDLE rpcHandle, GUID clientDsa)
{
	_clientDsa = clientDsa;
	_serverReplEpoch = DrsConnection::defaultReplEpoch;

	// Register the RetrieveSessionKey as RCP security callback. Mind the delegate lifecycle.
	RPC_STATUS status = RpcBindingSetOption(rpcHandle, RPC_C_OPT_SECURITY_CALLBACK, (ULONG_PTR)[this](void* rpcContext) { return this->RetrieveSessionKey(rpcContext); });

	Bind(rpcHandle);
	if (_serverReplEpoch != DrsConnection::defaultReplEpoch)
	{
...
void DrsConnection::Bind(IntPtr rpcHandle)
{
	...
	UUID clientDsaUuid = RpcTypeConverter::ToUUID(_clientDsa);
	...
	SetHandle((IntPtr)drsHandle);

	// Parse the server info
	DRS_EXTENSIONS_INT serverInfo = DRS_EXTENSIONS_INT(genericServerInfo);
	_serverSiteObjectGuid = RpcTypeConverter::ToGuid(serverInfo.siteObjGuid);
	_serverReplEpoch = serverInfo.dwReplEpoch;
	_serverCapabilities = serverInfo.dwFlags;
}
...
midl_ptr<DRS_EXTENSIONS_INT> DrsConnection::CreateClientInfo()
{
	...
	clientInfo->dwReplEpoch = _serverReplEpoch;
	return clientInfo;
}
...
array<ReplicationCursor^>^ DrsConnection::GetReplicationCursors(String^ namingContext)
{
	...
	DRS_HANDLE handle = handle.ToPointer();
...
midl_ptr<DRS_MSG_GETCHGREQ_V10> DrsConnection::CreateGenericReplicateRequest(midl_ptr<DSNAME> &&dsName, array<ATTRTYP>^ partialAttributeSet, ULONG maxBytes, ULONG maxObjects)
{
	...
	// Inset client ID:
	request->uuidDsaObjDest = RpcTypeConverter::ToUUID(_clientDsa);
	// Insert DSNAME
...
midl_ptr<DRS_MSG_GETCHGREQ_V10> DrsConnection::CreateReplicateSingleRequest(Guid objectGuid, array<ATTRTYP>^ partialAttributeSet)
{
	...
	request->uuidDsaObjDest = RpcTypeConverter::ToUUID(_serverSiteObjectGuid);
	return request;
}
...
{
	return ReplicateAllObjects(cookie, nullptr, DrsConnection::defaultMaxBytes, DrsConnection::defaultMaxObjects);
}

ReplicationResult^ DrsConnection::ReplicateAllObjects(ReplicationCookie^ cookie, ULONG maxBytes, ULONG maxObjects)
{
	return ReplicateAllObjects(cookie, nullptr, maxBytes, maxObjects);
}
...
{
	return ReplicateSingleObject(distinguishedName, nullptr);
}
...
ReplicaObject^ DrsConnection::ReplicateSingleObject(String^ distinguishedName, array<ATTRTYP>^ partialAttributeSet)
{
	...
	catch (DirectoryObjectNotFoundException^)
	{
		...
		bool objectExists = TestObjectExistence(distinguishedName);
		if (objectExists)
		{
...
{
	return ReplicateSingleObject(objectGuid, nullptr);
}
...
ReplicaObject^ DrsConnection::ReplicateSingleObject(Guid objectGuid, array<ATTRTYP>^ partialAttributeSet)
{
	...
	{
		// ReplicateSingleObject also exits with this error when access is denied, so we need to differentiate between these situations.
		bool objectExists = TestObjectExistence(objectGuid);
		if (objectExists)
		{
			// Force the validator to throw the DRA access denied exception.
			Validator::AssertSuccess(Win32ErrorCode::DS_DRA_ACCESS_DENIED);
...
midl_ptr<DRS_MSG_GETCHGREPLY_V9> DrsConnection::GetNCChanges(midl_ptr<DRS_MSG_GETCHGREQ_V10> &&request)
{
	...
	DRS_HANDLE handle = handle.ToPointer();
	const DWORD inVersion = MaxSupportedReplicationRequestVersion;
	DWORD outVersion = 0;
...
String^ DrsConnection::ResolveDistinguishedName(NTAccount^ accountName)
{
	...
	auto dn = TryResolveName(stringAccountName, DS_NAME_FORMAT::DS_NT4_ACCOUNT_NAME, DS_NAME_FORMAT::DS_FQDN_1779_NAME);
	if (dn == nullptr)
	{
...
String^ DrsConnection::ResolveDistinguishedName(SecurityIdentifier^ objectSid)
{
	...
	auto dn = TryResolveName(stringSid, DS_NAME_FORMAT::DS_SID_OR_SID_HISTORY_NAME, DS_NAME_FORMAT::DS_FQDN_1779_NAME);
	if (dn == nullptr)
	{
...
{
	auto stringAccountName = accountName->Value;
	auto stringGuid = TryResolveName(stringAccountName, DS_NAME_FORMAT::DS_NT4_ACCOUNT_NAME, DS_NAME_FORMAT::DS_UNIQUE_ID_NAME);
...
{
	auto stringSid = objectSid->ToString();
	auto stringGuid = TryResolveName(stringSid, DS_NAME_FORMAT::DS_SID_OR_SID_HISTORY_NAME, DS_NAME_FORMAT::DS_UNIQUE_ID_NAME);
	if (stringGuid == nullptr)
	{
		throw gcnew DirectoryObjectNotFoundException(stringSid, nullptr);
	}
...
{
	auto stringGuid = TryResolveName(userPrincipalName, DS_NAME_FORMAT::DS_USER_PRINCIPAL_NAME, DS_NAME_FORMAT::DS_UNIQUE_ID_NAME);
	if (stringGuid == nullptr)
	{
...
String^ DrsConnection::TryResolveName(String^ name, DS_NAME_FORMAT formatOffered, DS_NAME_FORMAT formatDesired)
{
	...
	// Perform RPC call
	auto reply = CrackNames(move(request));
...
{
	auto resolvedName = TryResolveName(distinguishedName, DS_NAME_FORMAT::DS_FQDN_1779_NAME, DS_NAME_FORMAT::DS_UNIQUE_ID_NAME);
...
bool DrsConnection::TestObjectExistence(Guid objectGuid)
{
	...
	auto resolvedName = TryResolveName(stringGuid, DS_NAME_FORMAT::DS_UNIQUE_ID_NAME, DS_NAME_FORMAT::DS_FQDN_1779_NAME);
	// Return true if and only if the object exists
	return resolvedName != nullptr;
}
...
{
	DRS_HANDLE ptr = handle.ToPointer();
...
DWORD DrsConnection::GetMaxSupportedReplicationRequestVersion()
{
	DWORD version = 5;

	if (_serverCapabilities & DRS_EXT::DRS_EXT_GETCHGREQ_V8)
	{
		version = 8;
	}

	if (_serverCapabilities & DRS_EXT::DRS_EXT_GETCHGREQ_V10)
	{
		version = 10;
	}
