...
{TR;
	...
	QString error;
	QString hashname = "???";
	if (passlink->Lookup(hashtype, lc7hashtype, error))
	{
...
{TR;
	...
	saved["gpu_enable"]=gpu_enable;
	saved["gpu_platform"]=(int)gpu_platform;
	saved["gpu_jtrindex"]=gpu_jtrindex;
	saved["jtrversion"] = jtrversion;
	saved["node_algorithm"]=node_algorithm;
	saved["preflight_node_algorithm"] = preflight_node_algorithm;
	saved["extra_opencl_kernel_args"] = extra_opencl_kernel_args;
...
{TR;
	...
	gpu_enable=saved["gpu_enable"].toBool();
	gpu_platform=(GPUPLATFORM)saved["gpu_platform"].toInt();
	gpu_jtrindex=saved["gpu_jtrindex"].toInt();
	...
	extra_opencl_kernel_args = saved.value("extra_opencl_kernel_args").toString();
