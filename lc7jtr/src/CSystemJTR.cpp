...
{TR;
	...
	*/
	...
	QVector<LC7GPUInfo> gi = CLC7JTR::GetSupportedGPUInfo(true);
	...
	foreach(LC7GPUInfo g,gi)
	{
		...
		// Add enable gpu
		AddOption(keys, 
			UUID_LC7JTRPLUGIN.toString()+QString(":enablegpu_%1_%2").arg(platform).arg(g.internal_index),
			QString("Enable %1 GPU #%2 (%3, %4)").arg(platform).arg(g.internal_index).arg(g.name).arg(g.vendor),
