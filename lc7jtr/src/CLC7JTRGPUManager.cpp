...
CLC7JTRGPUManager::CLC7JTRGPUManager()
{
	TR;
}
...
ILC7Interface *CLC7JTRGPUManager::GetInterfaceVersion(QString interface_name)
{
	if (interface_name == "ILC7GPUManager")
	{
		return this;
	}
	return nullptr;
...
bool CLC7JTRGPUManager::IsSupportedGPU(const LC7GPUInfo & gi)
{
	TR;
...
void CLC7JTRGPUManager::DetectOPENCL(void)
{
	TR;
	...
	if (ExecuteJTR(openclargs, opencl_out, opencl_err, opencl_retval))
	{
		...
		foreach(QString line, lines)
		{
			if (re_device.exactMatch(line))
			{
				...
				gi.internal_index = re_device.cap(1).toInt();
...
/*
void CLC7JTRGPUManager::DetectCUDA()
{
	TR;
...
*/
void CLC7JTRGPUManager::Detect(void)
{
	TR;
	DetectOPENCL();
	//DetectCUDA();
...
QVector<LC7GPUInfo> CLC7JTRGPUManager::GetGPUInfo() const
{
	TR;
	return m_gpuinfo;
}
